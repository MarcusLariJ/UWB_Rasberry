...
# %%
R_b =  0.015 # old value from datasheet 0.0009 #
R_r =  0.01 # old value from datasheet # 0.001
R_w = 0.0002
R_a = 0.004

Rcom = np.diag([R_b, R_r, R_w, R_a, R_a])
...
thres_rob = 14.5 # 99.5 % confidence else 12 for df=2
...
def run_sim_loop(j):
    ...
    for i in range(robot_N):
        ...
        # Setup robots
        robots.append( sim.robot_luft(x0=x0[i], path=pos[i], imu=y_IMU[i], dt=dt, id=111*(i+1), t=uwb_trans, R=Rcom) )
    ...
    for i in range(pos_len-1):
        ...
        hfunc.updateAllLuft(robots, [update_list]*3, [params]*3, i, dt, update_freq, meas_delay)
...
def main():
    ...
    robotData = sldat.RobotData(x_log, P_log, nis_imu_log, nis_rb_log, ref_pos, biases, self_ids, rb_ids)
    sldat.save_data(robotData, 'para')
